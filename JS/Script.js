...
let modalAlert, modalConfirmacion, modalConfirmarEliminar;
let signaturePad;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const modalConfirmacionElement = document.getElementById('staticBackdrop');
    const modalConfirmarEliminarElement = document.getElementById('modalConfirmarEliminar');
    
    if (modalAlertElement) {
        modalAlert = new bootstrap.Modal(modalAlertElement);
    }
    if (modalConfirmacionElement) {
        modalConfirmacion = new bootstrap.Modal(modalConfirmacionElement);
    }
    if (modalConfirmarEliminarElement) {
        modalConfirmarEliminar = new bootstrap.Modal(modalConfirmarEliminarElement);
    }
...
window.addEventListener("resize", () => {
    if (modalFirma && modalFirma.classList.contains("show")) {
        ajustarTamañoCanvas();
    }
});
...
// === Botón guardar ===
const guardarFirmaBtn = document.getElementById("guardarFirma");
if (guardarFirmaBtn) {
...
if (agregarEquipoBtn) {
    agregarEquipoBtn.addEventListener('click', function () {
        if (contadorEquipos >= MAX_EQUIPOS) {
            mostrarAlerta("¡Máximo 20 equipos por documento!");
            return;
        }
...
document.addEventListener('click', function (e) {
    if (e.target.closest('.remove-equipo') || e.target.closest('.remover-equipo')) {
        if (contadorEquipos > 1) {
            ...
            mostrarAlerta("¡Debe haber al menos un equipo!");
        }
    }
});
...
function checkFileCount() {
    ...
    if (files.length > 20) {
        mostrarAlerta("¡Solo se permiten 20 fotografías!", "Por favor, selecciona máximo 20 imágenes.");
        fileInput.value = '';
        return false;
    }
    return true;
...
if (evidenciasInput) {
    evidenciasInput.addEventListener('change', checkFileCount);
}
...
if (formulario) {
    formulario.addEventListener("submit", async (e) => {
        ...
        if (!formulario.checkValidity()) {
            mostrarAlerta("¡Completa todos los campos obligatorios!", "Por favor, revisa que todos los campos marcados con * estén completos.");
            e.stopPropagation();
            return;
        }
...
if (btnGenerar) {
    btnGenerar.addEventListener("click", async (e) => {
        ...
        try {
            ...
            if (equipos.length === 0) {
                mostrarAlerta("Debes ingresar al menos un equipo con Serial, Modelo y Descripción.");
                return;
            }
            ...
            console.error(err);
            mostrarAlerta("Error generando el archivo: " + err.message);
        }
    });
}
...
// =====================
function mostrarAlerta(titulo, mensaje = "Se ha detectado un problema en la información ingresada.") {
    const modalAlertTitle = document.getElementById('modalAlert');
    const modalAlertBody = document.getElementById('modalAlertBody');
    
    if (modalAlertTitle) modalAlertTitle.innerHTML = titulo;
    if (modalAlertBody) modalAlertBody.textContent = mensaje;
    if (modalAlert) modalAlert.show();
}