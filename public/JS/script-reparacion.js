       // =====================
        ...
        class SistemaReparacion {
            ...
            inicializarEventListeners() {
                ...
                if (evidencias) {
                    evidencias.addEventListener('change', () => this.validarArchivos());
                }
...
agregarRepuesto() {
    ...
    container.insertAdjacentHTML('beforeend', repuestoHTML);
            ...
            async agregarFotos(workbook, hoja) {
                ...
                // Tamaño máximo dentro de la celda A45
                const maxWidth = 250;   // ancho total disponible
                const maxHeight = 120;  // alto máximo
                const halfWidth = maxWidth / 2; // cada foto la mitad
                ...
                for (let i = 0; i < Math.min(archivos.length, 2); i++) {
                    ...
                    // Crear imagen TEMPORAL para obtener ancho/alto original
                    ...
                    const ratio = Math.min(halfWidth / w, maxHeight / h);
                    w = w * ratio;
                    h = h * ratio;

                    // Registrar imagen en el workbook
                    const imgId = workbook.addImage({
                        base64: base64,
                        extension: extension
                    });

                    // Calcular posición (A45 = col 0, row 44)
                    const col = i === 0 ? 0 : (halfWidth / 72); // 72px = 1 col aprox
                    const row = 44;
